    ...
    <>
      {adventureData.map((card, i) => {
        return (
          ...
          <div
            key={i}
            className={`bg-indigo-100 w-full h-4/5 rounded-md flex items-center justify-around dropShadow 2xl:pl-5 pl-10 ${cssOverride}`}
          >
            ...
            >
              <input
                ...
                type="checkbox"
                className={`${active} border-2  border-empatGray 2xl:w-7 2xl:h-7 w-5 h-5 rounded-md mt-5 2xl:mr-0 lg:mr-10 hover:bg-gray-400 cursor-pointer duration-150 transition-all`}
              ></input>
            </div>
          </div>
        );
